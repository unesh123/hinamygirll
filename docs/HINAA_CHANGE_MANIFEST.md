...
| Capability | Current state | Evidence and boundary |
|---|---|---|
| Imported VRM arm pose | **REPAIRED IN CODE AND TESTED** | The user’s fullscreen screenshot showed real VMC packets with an imported VRM still in a wide authored rest pose. HINAA now uses a stronger normalized shoulder → upper-arm → forearm generic relaxation arc without altering VRM binaries. Imported models retain a one-click original-pose recovery. |
| Imported-model centered view | **REPAIRED AND TESTED** | Both main upload and Avatar Lab import reset a newly imported model to the persisted portrait companion camera plus default front-facing relaxed presentation, rather than inheriting stale full-body state. |
| Research depth selection | **REPAIRED AND TESTED** | Explicit research now selects standard, deep, or exhaustive You.com effort based on the request; approval retains visibility before execution. HINAA never silently selects frontier/background mode. Official You.com guidance confirms higher effort means deeper source work and higher latency/cost. |
| Research presentation | **POLISHED AND TYPE-TESTED** | Detailed research cards now label selected effort, attributed-source count, and any unfinished background research task rather than making a deep-dive look identical to a normal search. |
