...
| Deep research | You.com-backed cited research, visible effort level, source/task cards, completed-source context workspace, and user-triggered private source save | Research report template, source scoring, source bundles, and project report export | You.com key for live web research | Required before live web research is executed; local project saving occurs only after the user clicks Save |
| Slides / reports / documents | Local project artifacts, PDF/DOCX/PPTX extraction, Markdown exports, and deterministic private project-report creation | DOCX/PPTX generation through a verified document pipeline, templates, local preview, and user-reviewed export queue | Local document generators; optional model brain for drafting | Creating local Markdown reports is user-triggered and local-only; export/send remains explicit |
...
| Coding / site builder | Code-help route, project/workspace, task tree, website blueprint, private Markdown report | Scoped project scaffold, test/build loop, artifact preview, approved local command runner | Selected brain plus local Node/Python tools | Required before file writes, commands, Git operations, or publishing |
...
| WhatsApp / email | Approval panel and browser/messaging intent surfaces; Windows companion boundary documented | Draft composition, recipient selection, message review, one-send confirmation, verified completion receipt | Official provider/API or user-authenticated browser; paired Windows companion for desktop control; WhatsApp Business API has separate eligibility | Required for every send/call; never infer recipient or send silently |
...
| System/device controls | No safe direct device agent in the web app; companion protocol boundary documented | Typed/voice proposal → OS-level command preview → final approval → signed local receipt/audit log | Trusted Windows companion, device-specific permissions, paired local runtime | Required for volume, brightness, processes, Wi-Fi, wallpaper, power, or file changes |
