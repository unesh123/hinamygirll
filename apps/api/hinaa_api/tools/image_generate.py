...
_REF_STOPWORDS = {"image", "images", "picture", "pictures", "photo", "photos", "pic",
...
def reference_from_query_result(search_result: Dict[str, Any], subject: str = "") -> Optional[str]:
    ...
    Vendor rank order is popularity-biased — exactly what produced "I asked
    for Mikasa, got a bridge" behavior. Scoring every candidate's title, alt
    text and filename against the subject tokens and keeping the strongest
    overlap pins the reference to what the user actually named.
    """
    ...
    for position, item in enumerate(images):
        ...
        # Nudge ties toward the vendor's own preference; a full match wins hard.
