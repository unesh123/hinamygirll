...
export function useToolRunner(
  ...
  useEffect(() => {
    ...
    const runTools = async () => {
      ...
      // Independent tool requests run in parallel: each updates only its own
      // activity entry through functional state updates, which are race-safe.
      await Promise.all(lastMessage.plan!.toolRequests.map((req) => runTool(req)));
    };

    const runTool = async (req: (typeof lastMessage.plan.toolRequests)[number]) => {
      try {
