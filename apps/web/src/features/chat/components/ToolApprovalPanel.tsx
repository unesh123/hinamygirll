import { Check, Loader2, ShieldAlert, X, Zap } from "lucide-react";
...
interface ToolApprovalPanelProps {
  ...
  /**
   * Autonomy mode. When true the user has granted standing consent in Settings,
   * so actions run without a per-action approval click and no Allow/Decline
   * buttons are offered.
   */
  autoRun?: boolean;
  onResolve: (
...
export function ToolApprovalPanel({ messageId, requests, activity = [], autoRun = false, onResolve }: ToolApprovalPanelProps) {
  ...
  return (
    <section aria-label="Proposed agent actions" style={{ display: "grid", gap: 8, marginTop: 10 }}>
      {requests.map((request, index) => {
        ...
        const tone = complete ? "#34d399" : declined ? "#94a3b8" : failed ? "#fda4af" : autoRun ? "#7dd3fc" : "#fbbf24";
        const heading = autoRun
          ? complete
            ? `Done · ${request.toolName}`
            : failed
            ? `Failed · ${request.toolName}`
            : declined
            ? `Skipped · ${request.toolName}`
          ...
          <div key={`${request.toolName}-${index}`} style={{ border: `1px solid ${tone}44`, borderRadius: 10, background: "rgba(15,23,42,.58)", padding: "10px 11px" }}>
            <div style={{ display: "flex", alignItems: "flex-start", gap: 8 }}>
              <span style={{ display: "grid", placeItems: "center", width: 25, height: 25, flexShrink: 0, borderRadius: 8, color: tone, background: `${tone}1a` }}>
                {running || (autoRun && pending) ? <Loader2 size={14} className="spin" /> : complete ? <Check size={14} /> : autoRun ? <Zap size={14} /> : <ShieldAlert size={14} />}
              </span>
              <div style={{ minWidth: 0, flex: 1 }}>
                <strong style={{ display: "block", color: "#f8fafc", fontSize: 12 }}>{heading}</strong>
