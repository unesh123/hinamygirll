...
try {
  ...
  await mobilePage.goto("http://127.0.0.1:5173/", { waitUntil: "networkidle", timeout: 60000 });
  await mobilePage.waitForTimeout(3000);

  await mobilePage.screenshot({ path: path.join(OUT, "mobile-01-chat.png") });
  console.log("Captured mobile-01-chat.png");

  // Switch to 3D avatar tab
  const avatarTab = mobilePage.locator('[data-testid="mobile-tab-avatar"]').first();
  if (await avatarTab.count()) {
    await avatarTab.click();
    await mobilePage.waitForTimeout(2000);
    await mobilePage.screenshot({ path: path.join(OUT, "mobile-02-avatar.png") });
    console.log("Captured mobile-02-avatar.png");
  }

  // Switch back to Chat
  const backBtn = mobilePage.locator('[data-testid="mobile-back-to-chat"]').first();
  if (await backBtn.count()) {
    await backBtn.click();
    await mobilePage.waitForTimeout(1000);
    await mobilePage.screenshot({ path: path.join(OUT, "mobile-03-back-chat.png") });
    console.log("Captured mobile-03-back-chat.png");
  }

  await mobileCtx.close();
} catch (e) {
  console.log("Mobile smoke failed:", e.message);
}
